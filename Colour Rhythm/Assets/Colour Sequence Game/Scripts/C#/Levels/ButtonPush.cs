...
public class ButtonPush : MonoBehaviour {
    ...
    public Level1 level1; //This holds the GameObject of the GameManager so we can send messages more efficiently
    ...
    void OnMouseDown() //This function is called whenever you push down on the left mouse button, inside the objects collider
    {
        ...
        { //The code inside the brackets can only be done if canClick is equal to true you could also write it if(canClick == true)
            ...
            level1.StartCoroutine("CheckSequenceButton", id); //When we click the button we want to check if it is the correct one. So we send over our id over to the GameManager to see if it is.
