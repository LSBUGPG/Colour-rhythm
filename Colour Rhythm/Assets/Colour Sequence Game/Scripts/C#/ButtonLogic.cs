...
using UnityEngine.UI; //We need to import Unity's UI tools to be able to use them
...
public class ButtonLogic : MonoBehaviour 
{
	...
	public GameLogic gameManager; //This holds the GameObject of the GameManager so we can send messages more efficiently
	...
	{
		if(canClick){ //The code inside the brackets can only be done if canClick is equal to true you could also write it if(canClick == true)
			...
			gameManager.StartCoroutine("CheckSequenceButton", id); //When we click the button we want to check if it is the correct one. So we send over our id over to the GameManager to see if it is.
